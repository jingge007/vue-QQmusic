...
function findIndex(list, song) {     // 当歌曲的播放模式是随机播放模式，获取打乱的歌曲列表的索引
  return list.findIndex((item) => {
    return item.id === song.id
  })
}

export const selectPlay = function ({commit, state}, {list, index}) {     // 选择歌曲播放
  commit(types.SET_PLAYING_STATE, true)           // 播放器播放状态
  commit(types.SET_FULL_SCREEN, true)            // 播放器展开状态
  if (state.mode === 2) {                       // 当歌曲为随机播放时，将歌曲的数据列表打乱
    let randomList = myArray.shuffle(list)
    commit(types.SET_PLAYLIST, randomList)           // 播放歌曲列表
